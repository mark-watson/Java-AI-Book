# Web scraping / info-gathering example
...
- Tests / run:        `make test` (some tests hit geonames/weather endpoints — require network)
...
The Stop hook (`.claude/hooks/java-stop.sh`) runs `spotless:check` + `test-compile` when Claude finishes its turn. Full tests are excluded because some require live network access.

## Project structure
- Primary packages: `com.markwatson.web_scraper` and `org.geonames`.
- Uses `jsoup` for HTML parsing and `jdom2` for XML parsing.
