...
- Tests / run:        `make test` (requires `OPENAI_API_KEY` env var and network)
...
The Stop hook (`.claude/hooks/java-stop.sh`) runs `spotless:check` + `test-compile` when Claude finishes its turn. Full tests are excluded because they require network access to the OpenAI API.

## Project structure
- Main class: `com.markwatson.openai.OpenAICompletions`
- Uses the official `com.openai:openai-java` SDK.
...
## Testing notes
- Tests hit the live OpenAI API — set `OPENAI_API_KEY` and expect network I/O.
