# DBPedia Named Entity Recognition library
...
- All Maven commands in the inner loop use `-o -q` (offline + quiet) — run `make install` once to prime the local repo and install the artifact into `~/.m2` so downstream projects (`kgn`, `kgc`) can resolve it.

## Build & check
- Fast compile check: `make check`  (`mvn -o -q test-compile`)
- Format:             `make fmt`
- Format check:       `make fmt-check`
- Tests / run:        `make test` (requires network — hits DBPedia SPARQL endpoint)
...
The Stop hook (`.claude/hooks/java-stop.sh`) runs `spotless:check` + `test-compile` when Claude finishes its turn. Full tests are excluded because they require live network access to DBPedia.

## Project structure
- Package: `com.markwatson.ner_dbpedia`
- Main classes: `TextToDbpediaUris` (entity extraction), `NerMaps` (name/URI lookup tables).
- Consumed by `kgn` (Knowledge Graph Navigator) and `kgc` (Knowledge Graph Creator) — `make install` must be run once so the artifact lands in `~/.m2`.
