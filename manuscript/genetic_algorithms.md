...
Please note, dear reader, that this material is **dated**. While it still correctly describes the basics of how GAs work, I have not personally used GAs for many years. In the last decade deep learning has largely supplanted GAs for many tasks. In addition GAs can be implemented with more powerful modern tools such as Keras and TensorFlow, and in the last five years LLMs have supplanted GAs for many tasks. 
...
Previously, we used a separate `ChromosomeComparator` class to sort the population. Now, we use standard Java 8 stream/lambda comparator logic inside the `sort` method:
...
This avoids needing a separate class.
