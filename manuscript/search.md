...
{width: "80%"}
![Architecture diagram](images/search-architecture.png)
...
Here are some hands-on practice problems to help deepen your understanding of the search algorithms and frameworks covered in this chapter:

1. **Enhancing Maze Search Statistics (Easy)**
   - **Goal:** In the current implementation, both [BreadthFirstSearchEngine.java](file:///Users/markwatson/GITHUB/Java-AI-Book/source-code/search/src/main/java/search/maze/BreadthFirstSearchEngine.java) and [DepthFirstSearchEngine.java](file:///Users/markwatson/GITHUB/Java-AI-Book/source-code/search/src/main/java/search/maze/DepthFirstSearchEngine.java) compute a search path, but they do not report the search effort. Modify the abstract base class [AbstractSearchEngine.java](file:///Users/markwatson/GITHUB/Java-AI-Book/source-code/search/src/main/java/search/maze/AbstractSearchEngine.java) to track a `visitedCount` counter. Increment this counter each time a location is evaluated, and print the total count when a search finishes. Compare the number of visited states for both depth-first and breadth-first searches on mazes of different sizes.
   
2. **Implementing Diagonal Moves in Maze Search (Medium)**
   - **Goal:** The search operators in [getPossibleMoves](file:///Users/markwatson/GITHUB/Java-AI-Book/source-code/search/src/main/java/search/maze/AbstractSearchEngine.java#L59-L76) only allow 4-way movement (up, down, left, right). Extend this method to allow 8-way movement, including diagonal moves. Be sure to check that diagonal paths do not "cut through" corners where obstacles are adjacent. Compare the resulting paths generated by [BreadthFirstSearchEngine.java](file:///Users/markwatson/GITHUB/Java-AI-Book/source-code/search/src/main/java/search/maze/BreadthFirstSearchEngine.java) before and after this change.
   
3. **Graph Heuristic Search (A* Search) (Medium/Hard)**
   - **Goal:** The class [AbstractGraphSearch.java](file:///Users/markwatson/GITHUB/Java-AI-Book/source-code/search/src/main/java/search/graph/AbstractGraphSearch.java) stores coordinates for each node (`node_x` and `node_y`) and distances between linked nodes. Implement a new class `AStarGraphSearch` that extends [AbstractGraphSearch.java](file:///Users/markwatson/GITHUB/Java-AI-Book/source-code/search/src/main/java/search/graph/AbstractGraphSearch.java). Use Euclidean distance as the heuristic function to estimate the remaining distance to the goal node. Test your implementation on the provided graph and verify that it finds the shortest path while exploring fewer nodes than standard breadth-first search.
   
4. **Improving Tic-Tac-Toe Position Evaluation (Hard)**
   - **Goal:** The current heuristic function in [positionEvaluation](file:///Users/markwatson/GITHUB/Java-AI-Book/source-code/search/src/main/java/search/game/TicTacToe.java#L41-L66) is quite simple: it gives minor extra points for occupying the center square and checks for wins or losses. Improve this heuristic by scanning all rows, columns, and diagonals for two-in-a-row opportunities (i.e., two squares marked by the same player and one blank square) and scoring them appropriately. Test your new evaluator against the default computer player using the [TicTacToe.java](file:///Users/markwatson/GITHUB/Java-AI-Book/source-code/search/src/main/java/search/game/TicTacToe.java) class and see if the program plays a stronger game.
